...
final class KnowledgeStore {
  ...
  func loadKnowledge(id: String) async {
    ...
    do {
      selectedKnowledge = try await client().fetchKnowledge(id: id)
    } catch {
      errorMessage = error.localizedDescription
    }
  }
  ...
  func createKnowledge(_ draft: DashboardKnowledgeDraft) async -> DashboardKnowledge? {
    ...
    do {
      let createdKnowledge = try await client().createKnowledge(draft)
  ...
  func updateKnowledge(
    ...
    do {
      let updatedKnowledge = try await client().updateKnowledge(id: id, draft: draft)
  ...
  func deleteKnowledge(id: String) async {
    ...
    do {
      try await client().deleteKnowledge(id: id)
  ...
  private func client() throws -> DashboardAPIClient {
    guard let configuration else {
      throw DashboardStoreError.notConfigured
    }

    return DashboardAPIClient(configuration: configuration)
  }
}
