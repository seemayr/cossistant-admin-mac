...
struct SidebarView: View {
  ...
  var body: some View {
    ...
    .searchable(text: $model.searchText, prompt: "Search inbox")
  }
  ...
  private var contextSection: some View {
    Section("Connected Website") {
      SidebarStatRow(
        label: "Open",
        value: model.conversations.filter { $0.status == .open }.count,
        tint: .green
      )

      SidebarStatRow(
        label: "Resolved",
        value: model.conversations.filter { $0.status == .resolved }.count,
        tint: .secondary
      )

      SidebarStatRow(
        label: "Spam",
        value: model.conversations.filter { $0.status == .spam }.count,
        tint: .red
      )
