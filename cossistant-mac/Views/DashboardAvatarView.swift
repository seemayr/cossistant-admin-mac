...
enum DashboardAvatarRole: Hashable, Sendable {
  case person
  case ai
}
...
struct DashboardAvatarView: View {
  ...
  var role: DashboardAvatarRole = .person
...
struct DashboardAvatarPreviewButton: View {
  let name: String
  let imageURL: URL?
  let seed: String
  var size: CGFloat = 38
    ...
    Group {
      if imageURL != nil {
        ...
        .sheet(isPresented: $isPresentingPreview) {
          DashboardAvatarPreviewSheet(name: name, imageURL: imageURL)
        }
      } else {
        DashboardAvatarView(
          name: name,
...
private struct DashboardAvatarPreviewSheet: View {
  let name: String
  let imageURL: URL?
